	...
	if (!process.env.DISABLE_ANALYTICS) {
		...
		console.info(
			chalk.green("[ANALYTICS]") +
				` manoos has non-invasive analytics for Jip. These analytics include the platform manoos is running on, your username, and whether you have configured Discord webhooks and the Telegram bots for notifications.`
		);
		console.info(
			chalk.green("[ANALYTICS]") +
		...
		// Submit
		fetch(
			`https://manoos.jip-fr.workers.dev/?username=${
